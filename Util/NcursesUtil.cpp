...
void ncurses_util::InitAllPossibleColorPairs() {
    ...
    for (bg = 0; bg < 16; bg++) {
        for (fg = 0; fg < 16; fg++) {
            ...
            init_pair((short)color_pair, ncurses_util::CursColor(fg), ncurses_util::CursColor(bg));
        }
    }
}
...
void ncurses_util::AddText(Vec2i screen_coords, const std::string& text, int fg_color, int bg_color) {
    SetColor(fg_color, bg_color);
    mvaddstr(screen_coords.y, screen_coords.x, text.c_str());
    UnsetColor(fg_color, bg_color);
}

void ncurses_util::AddTextColorPair(Vec2i screen_coords, const std::string& text, int color_pair) {
...
void ncurses_util::ConsoleWriter::AddLine(const std::string& text, int fg_color, int bg_color) {
